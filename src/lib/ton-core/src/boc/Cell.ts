...
export class Cell {
    ...
    static fromBoc(src: Buffer) {
        // console.log('ton start deserializeBoc:');
    ...
    constructor(opts?: { exotic?: boolean, bits?: BitString, refs?: Cell[] }) {
        ...
        // fileLog('Cell Hashes:' + this.type + ' ' + this.mask.value + ' ' + this.mask.level);
        // fileLog('[' + hashes.map(h => h.toString('hex')).join(',\n') + ']')
