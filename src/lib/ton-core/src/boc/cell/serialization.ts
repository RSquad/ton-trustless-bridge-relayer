...
// import {fileLog} from "../../../../trustless-ton/file-logs";
...
function readCell(reader: BitReader, sizeBytes: number) {
    ...
    // console.log(dataBytesize, paddingAdded);
    // fileLog(dataBytesize + '', paddingAdded + '', d1 + '', d2 +'', refsCount + '', hashesSize + '', depthSize + '');
    ...
    if (dataBytesize > 0) {
        if (paddingAdded) {
            ...
            // bits = reader.loadPaddedBits(dataBytesize * 8 + hashesSize * 8 + depthSize * 8);
        } else {
            // bits = reader.loadBits(dataBytesize * 8 + hashesSize * 8 + depthSize * 8);
            bits = reader.loadBits(dataBytesize * 8);
        }
    }

    // fileLog(bits._data.subarray(bits._offset >> 3, (bits._offset + bits.length) >> 3).toString('hex'))
...
export function deserializeBoc(src: Buffer) {
    ...
    for (let i = cells.length - 1; i >= 0; i--) {
        ...
        // console.log(cells[i].result?.hash(0).toString('hex'));
    }
